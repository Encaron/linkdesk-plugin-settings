...
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`settings`）。当前版本 `1.0.17`。

## 1. 这是什么

设置——外观 / 字体 / 语言 / 快捷键 / 插件管理。**它就是软件的控制面板**，只是以插件形态存在。

**用户在哪看到它**：图标栏底部按钮 ＋ 右栏（`location: "auxiliarybar"`）＋ 悬浮面板（`floatingPanel`）。
